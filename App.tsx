  ...
  const renderTopicSelection = () => (
    <div className="max-w-4xl mx-auto px-4 py-12 text-center animate-in fade-in duration-700">
      ...
      <p className={`mb-12 max-w-2xl mx-auto ${isDarkMode ? 'text-slate-300' : 'text-gray-600'}`}>
        Domine as estruturas de dados fundamentais e a lógica de programação com desafios teóricos de alto nível.
      </p>
        ...
        {topics.map(topic => (
          ...
          >
            <div className={`w-14 h-14 rounded-2xl mb-4 flex items-center justify-center text-2xl 
              ${topic === 'Pilhas e Filas' ? 'bg-blue-100 text-blue-600' : 
                topic === 'Recursividade' ? 'bg-green-100 text-green-600' : 
                topic === 'Listas' ? 'bg-orange-100 text-orange-600' : 
                topic === 'Vetores e Matrizes' ? 'bg-purple-100 text-purple-600' : 
                topic === 'Árvores, Heaps e Hashing' ? 'bg-red-100 text-red-600' :
                'bg-emerald-100 text-emerald-600'}`}>
              <i className={
                topic === 'Pilhas e Filas' ? 'fas fa-layer-group' : 
                topic === 'Recursividade' ? 'fas fa-redo' : 
                topic === 'Listas' ? 'fas fa-list-ul' : 
                topic === 'Vetores e Matrizes' ? 'fas fa-th' : 
                topic === 'Árvores, Heaps e Hashing' ? 'fas fa-tree' :
                'fas fa-code'
              }></i>
            </div>
